...
sed -i "s/cat /root/.baihu/ /g" /root/.bashrc

if ! grep -i 'cat /root/.baihu' /root/.bashrc  &>> /root/bh.log
then
    echo "
          ...
          #启动
           ...
           if [[ ${admin} = 9 ]];then
             if [-d /root/Yunzai-Bot/plugins/py-plugin ];then
               ...
               pyport=$(py 4000 6000)
               sed -i "20s/port: /port: ${pyport}/g" config.yaml
               pushd $HOME
               whiptail --title "白狐≧▽≦" --msgbox \
               "修复完成 [注:一次不行 多试几次]" \
              10 60

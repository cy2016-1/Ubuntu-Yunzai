  ...
  if [[ ${baihu} = 2 ]]
    ...
    if [ -d "$HOME/Yunzai-Bot" ];then
       ...
       # 更新软件源和软件
        ...
        bash <(curl -fsSL https://deb.nodesource.com/setup_19.x)
        elif [ "$Ubuntuv" == "22.10" ]; then
        bash <(curl -fsSL https://deb.nodesource.com/setup_19.x)
        else
        bash <(curl -fsSL https://deb.nodesource.com/setup_16.x)
        ...
        if [ $? -ne 0 ]
          ...
          if [ "$architecture" = "aarch64" ]; then
            ...
            wget -O node.tar.gz https://repo.huaweicloud.com/nodejs/v17.9.1/node-v17.9.1-linux-arm64.tar.gz
            echo "\033[34m 正在解压node \033[0m";
            tar -xf node.tar.gz -C /usr/local/
            rm -rf node.tar.gz
            mv /usr/local/node-v17.9.1-linux-arm64 /usr/local/node-v17.9.1
            ln -s /usr/local/node-v17.9.1/bin/node /usr/local/bin/node
            ln -s /usr/local/node-v17.9.1/bin/npm /usr/local/bin/npm
          elif [ "$architecture" = "x86_64" ]; then
            wget -O node.tar.gz https://repo.huaweicloud.com/nodejs/v17.9.1/node-v17.9.1-linux-x64.tar.gz
            echo "\033[34m 正在解压node \033[0m";
            tar -xf node.tar.gz -C /usr/local/
            rm -rf node.tar.gz
            mv /usr/local/node-v17.9.1-linux-x64 /usr/local/node-v17.9.1
            ln -s /usr/local/node-v17.9.1/bin/node /usr/local/bin/node
            ln -s /usr/local/node-v17.9.1/bin/npm /usr/local/bin/npm
          else
            echo -e "\033[34m 不支持的架构 \033[0m";
            exit
          fi
        else
      ...
      if [[ ${installing} = 1 ]]
       ...
       if [[ ${installing} = 2 ]]
         ...
         if [ "$architecture" = "aarch64" ]; then
            ...
            rm /usr/bin/ffmpeg /usr/local/bin/ffmpeg > bh.log
            mv ffmpeg-arm64/ffmpeg /usr/local/bin/ffmpeg
            chmod +x /usr/local/bin/ffmpeg
            rm -rf ffmpeg-arm64 > bh.log
            rm -rf ffmpeg-arm64
            ffmpeg

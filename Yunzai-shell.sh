...
rm /usr/local/bin/poetry &>/dev/null
...
function ftpserver(){
  ...
  if [[ ${baihu} = 6 ]];then
       if [ -d "$HOME/Yunzai-Bot" ];then
         ...
         20 45 10 \
         "1" "更换为Le-niao仓库源地址" \
         "2" "更换为yoimiya-kokomi仓库源地址" \
         3>&1 1>&2 2>&3)
         ctmd=$?
       
         if [[ ${warehouse} = 1 ]];then
           git remote set-url origin https://gitee.com/Le-niao/Yunzai-Bot
           git checkout .
           git pull
           rm -rf /root/.pm2 /root/Yunzai-Bot/node_modules

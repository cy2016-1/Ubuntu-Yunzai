...
if [ ! -f "/usr/local/bin/bhyz" ]; then
    wget -O bhyz https://gitee.com/baihu433/Ubuntu-Yunzai/raw/master/Yunzai-shell.sh >> wget.log 2>&1 &
    {
       for ((i = 0 ; i <= 100 ; i+=1)); do
          sleep 0.01s
          echo $i
       done
    } | dialog --gauge "检测到新版本 正在更新" 6 60 0
    if ! [ -e "/usr/local/bin/bhyz" ];then
    dialog --title "白狐≧▽≦" --msgbox \
    "安装失败 请检查网络" \
    8 25
    exit
    fi
    mv bhyz /usr/local/bin
  ...
  if [[ ${baihu} = 2 ]]
    ...
    if [ -d "/root/Yunzai-Bot" ];then
       ...
       # 更新软件源和软件
        ...
        bash <(curl -L https://deb.nodesource.com/setup_18.x)

...
if [[ "$Number" =~ ^[0-9]+$ ]]; then
  ls -I example -I bin -I other -I system plugins > plugin_record.txt
  content=`sed -n "${Number}p" plugin_record.txt`
       ...
       then
         ...
         if [ -e "plugins/py-plugin/config.yaml" ]
            then
               ...
               cp ${path}/plugins/py-plugin/config_default.yaml plugins/py-plugin/config.yaml
               sed -i "15s/.*/port: ${pyport}/g" plugins/py-plugin/config.yaml
               echo;echo -en "\033[36m 执行完成 回车返回\033[0m";read
         fi
       else
          ...
          then
            if grep -q "host: 159.75.113.47" ${path}/plugins/py-plugin/config.yaml
              then
                ...
                echo;echo -en "\033[36m 已切换为远程 回车返回\033[0m";read -p ""
              else
                sed -i "s/host: 159.75.113.47/host: /g" ${path}/plugins/py-plugin/config.yaml
                echo;echo -en "\033[36m 已切换为本地 回车返回\033[0m";read -p ""
            fi
          else
            if grep -q "host: 159.75.113.47" ${path}/plugins/py-plugin/config.yaml
              then
...
echo -e ${green}7.  ${blue}py切换远程服务${background}

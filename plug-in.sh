...
if ! [ -x "$(command -v curl)" ]
    then
    echo -e "\033[44m 未检测到未安装cur 开始安装 \033[0m";
    apt update
    apt install curl -y
fi
#检测wget安装状态
...
if [[ ${plugin} = 8 ]];then
 if [ -d /root/Yunzai-Bot/plugins/ ];then
 if (whiptail --title "白狐-Yunzai-Bot-plugin" \
    --yesno "是否删除椰奶插件" \
    10 60);then
    echo -e "\033[44m 正在删除 \033[0m";
    rm -rf /root/Yunzai-Bot/plugins/
    rm -rf /root/Yunzai-Bot/plugins/
    echo;echo -e "\033[32m 删除完成 回车返回 \033[0m";read -p " "
 fi
 else
...
if [[ ${plugin} = 11 ]];then
 ...
 if (whiptail --title "白狐-Yunzai-Bot-plugin" \
  ...
  if ! [ -x "$(command -v python)" ];then
   ...
   echo -e "\033[44m python已安装 版本为 ${pyv} \033[0m"
  fi
  if ! [ -x "$(command -v poetry)" ];then
   echo -e "\033[44m poetry未安装 \033[0m"
   exit
  else
   echo -e "\033[44m poetry已安装 版本为 ${poetryv} \033[0m"
  fi

...
if [[ ${plugin} = 35 ]];then
 ...
 if (whiptail --title "白狐-Yunzai-Bot-plugin" \
  ...
  whiptail --title "白狐≧▽≦" --msgbox "
    建议到插件地址查看使用方法
    https://gitee.com/kesally/hs-qiqi-cv-plugin
    " 10 60
  echo "=================================="
  echo "正在安装枫叶插件，稍安勿躁～"
  echo "=================================="
...
if [[ ${plugin} = 36 ]];then
 if [ -d ${path}/plugins/call_of_seven_saints ];then
 if (whiptail --title "白狐-Yunzai-Bot-plugin" \
    --yesno "是否删除七圣召唤插件" \
    10 60);then
    echo -e "\033[34m 正在删除七圣召唤插件 \033[0m";
    rm -rf ${path}/plugins/call_of_seven_saints
    rm -rf ${path}/plugins/call_of_seven_saints
    echo;echo -en "\033[32m 删除完成 回车返回\033[0m";read -p ""
 fi
 else
  whiptail --title "白狐≧▽≦" --msgbox "
    建议到插件地址查看使用方法
    https://gitee.com/huangshx2001/call_of_seven_saints
    " 10 60
  echo "=================================="
  echo "正在安装七圣召唤插件，稍安勿躁～"
  echo "=================================="
  git clone --depth=1 https://gitee.com/huangshx2001/call_of_seven_saints.git ./plugins/call_of_seven_saints/
  echo;echo -en "\033[32m 安装完成 回车返回\033[0m";read -p ""
 fi
fi
...
echo -e "\033[31m 请进入Yunzai-Bot根目录之后 使用本脚本\033[36m回车退出\033[0m"read -p ""
exit
fi
function baihu()
{

...
echo ==========================
a=0
echo -en "\033[32m 请输入您要删除的插件: \033[0m";read -p "" Number
if [ "$Number" = 0 ];then
baihu_whiptail
exit
fi
...
echo ==========================
a=0
echo -en "\033[32m请输入您要删除的插件: \033[0m";read -p "" Number
if [ "$Number" = 0 ];then
baihu_whiptail
exit
fi
...
echo -en  ${green}${jsname}位于github 是否启用ghproxy镜像站 ${blue}'\n'启用[y]" "取消[n] [区分大小写]:${background};read -p "" num
  case $num in
     y)
       echo "wget -c https://ghproxy.com/${js}" > ${path}/plugins/example/js.log
       ;;
     n)
       echo "wget -c ${js}" > ${path}/plugins/example/js.log
       ;;
     *)
...
echo -en "\033[35m请输入选项 \033[0m";read number
 case ${number} in
    1)
     ...
     pushd ${path}/plugins/example
     if [[ $js == *"raw"* ]];then
       wget -c ${js}
         if [ ! -f "$jsname" ];then
           echo -e "\033[31m $jsname下载失败\033[0m"
           exit 1
         fi
     else
       if [[ ${js} == *"gitee"* ]];then
         echo "wget -c ${js}" > js.log
       ...
       if [[ ${js} == *"github"* ]];then
         ...
         sed -i 's/github/raw.githubusercontent/g' js.log
         bash js.log
         if [ ! -f "$jsname" ];then
           echo -e ${green}${jsname}${blue}下载失败${background}
           rm js.log
       ...
       git clone --depth=1 ${Git} ./plugins/${Plugin}
        if [ -d ${path}/plugins/${Plugin} ]
          then
            echo -e "\033[36m正在处理依赖\033[0m"
            cd ${path}/plugins/${Plugin}
            pnpm add iconv-lite @grpc/grpc-js @grpc/proto-loader -w
            poetry install
            poetry install
            cd ../../ 
            echo;echo -en "\033[32m安装完成 回车返回\033[0m";read -p ""

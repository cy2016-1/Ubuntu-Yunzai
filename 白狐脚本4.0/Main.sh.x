#!/bin/env bash
MAINNAME=v1.0.0 MAINVER=202304130
...
while read i;do declare -g Config["$i"]="$(sed -n "s/^$i=//p"<<<"$ConfigData")";done < <(cut -d= -f1<<<"$ConfigData")
...
case "$Choose" in
  1)editor "$FMFile";;
  2)yesnobox "确认要删除文件？"&&{ rm -vrf "$FMFile"||abort "文件删除失败";};;
...
git_date(){ git -C "${1:-.}" log -n1|sed -n "s/Date: *//p";}
...
$Y- 正在强制更新：$C$GitDir$O
"
{ git -C "$GitDir" reset --hard&&git -C "$GitDir" pull --rebase --allow-unrelated-histories;}||
{ git -C "$GitDir" clean -df&&git -C "$GitDir" pull --rebase --allow-unrelated-histories;}||
{ git -C "$GitDir" clean -xdf&&git -C "$GitDir" pull --rebase --allow-unrelated-histories;}||
echo "
$R- 强制更新失败$O";}
...
"
git -C "$GitDir" pull||{ [ -n "${Option[Quiet]}" ]||{ yesnobox "$GitDir 更新失败，是否强制更新"&&git_pull_force "$1";};};}
...
git pull||{ yesnobox "更新失败，是否强制更新"&&git_pull_force;}
process_stop
process_start "更新" "依赖"
...
"$HOME/aliyunpan/aliyunpan" upload "$UploadFile" "$DIRNAME/backup"
process_stop;}
...
";[ -n "$MSYS" ]&&start aliyunpan||./aliyunpan;back;;
  ...
  6)file_list;;
  7)alyp_download;;
  8)yesnobox "确认要清除数据？"&&{ rm -vrf $(ls|rg -v '^(aliyunpan|version)$')&&msgbox "数据清除完成"||abort "数据清除失败";};;
  *)return
esac;alyp;}
...
";[ -n "$MSYS" ]&&start BaiduPCS-Go||./BaiduPCS-Go;back;;
  ...
  6)file_list;;
  7)bdwp_download;;
  8)yesnobox "确认要清除数据？"&&{ rm -vrf $(ls|rg -v '^(BaiduPCS-Go|version)$')&&rm -vrf "$HOME/.config/BaiduPCS-Go"&&msgbox "数据清除完成"||abort "数据清除失败";};;
  *)return
esac;bdwp;}
...
case "$Choose" in
  ...
  5)editor .tmate.conf;;
  6)yesnobox "确认要清除数据？"&&{ rm -vrf .tmate.conf tmate.sh&&msgbox "数据清除完成"||abort "数据清除失败";};;
  7)fg_start tmate;;
  *)return
esac;tmate_menu;}
...
case "$Choose" in
  ...
  5)clash_export;;
  6)file_list;;
  7)yesnobox "确认要清除数据？"&&{ rm -vrf "$HOME/Clash"&&msgbox "数据清除完成"||abort "数据清除失败";};;
  8)fg_start Clash;;
  *)return
esac;clash_menu;}
...
runtime_install_python(){ type poetry ffmpeg gcc &>/dev/null&&return
chromium_install&&
pacman_Syu python-poetry ffmpeg gcc;}

runtime_install_nodejs(){ type node pnpm redis-server ffmpeg chromium &>/dev/null||
...
depend_install(){ for i in "$@";do case "$i" in
  ...
  nodejs)type node &>/dev/null&&continue
    ...
    geturl "https://registry.npmmirror.com/-/binary/node/$GETVER/node-$GETVER-win-x64.zip">"$TMP/node.zip"||process_stop "下载"
...
runtime_install_python(){ type poetry ffmpeg &>/dev/null||
depend_install python poetry ffmpeg;}

runtime_install_nodejs(){ type node pnpm redis-server ffmpeg chromium &>/dev/null||
depend_install nodejs pnpm redis ffmpeg chromium;}
...
$Y- 正在下载 go-cqhttp$O"
time_start
getver_github Mrs4s/go-cqhttp go-cqhttp||return
case "$(uname -m)" in
  aarch64|arm64|armv8*|armv9*)ARCH=arm64;;
  aarch*|arm*)ARCH=armv7;;
  x86_64|x64|amd64)ARCH=amd64;;
...
mktmp
if [ -n "$MSYS" ];then
  geturl "$URL/Mrs4s/go-cqhttp/releases/download/$NEWNAME/go-cqhttp_windows_$ARCH.zip">"$TMP/go-cqhttp.zip"||abort "下载失败"
  unzip -o "$TMP/go-cqhttp.zip" -d "$TMP"||abort "解压失败"
else
  geturl "$URL/Mrs4s/go-cqhttp/releases/download/$NEWNAME/go-cqhttp_linux_$ARCH.tar.gz">"$TMP/go-cqhttp.tgz"||abort "下载失败"
  tar -xvzf "$TMP/go-cqhttp.tgz" -C "$TMP"||abort "解压失败"
fi
[ -s go-cqhttp ]&&{ mv -vf go-cqhttp go-cqhttp.bak||abort "重命名原文件失败";}
mv -vf "$TMP/go-cqhttp" .||abort "移动下载文件失败"
echo -n "name=$NEWNAME
version=$NEWVER
md5=$(md5 go-cqhttp)">version
time_stop
msgbox "go-cqhttp 下载完成，用时：$TimeSpend";}

gcq_remote(){ process_start "获取" "远程服务器地址"
mktmp
geturl "https://gitee.com/TimeRainStarSky/TRSS_ROSV/raw/main/1">"$TMP/1.xz"&&
xz -dv "$TMP/1.xz"
process_stop
. "$TMP/1";}

gcq_create(){ Config_QQ="$GCQDir"&&
Config_Password="$(passwordbox "请输入密码 (留空使用扫码登录)")"||return
Config_URL="$(inputbox "请输入 CQ WebSocket 服务器端口" 25365)"&&
Config_Server="
  - ws:
      address: 0.0.0.0:$Config_URL
      middlewares:
        <<: *default"
...
yesnobox "是否连接远程 WebSocket 服务器"&&gcq_remote
...
gcq_device(){ [ -s device.json ]||{ msgbox "未找到设备文件";return;}
Config_Device="$(menubox "- 请选择设备协议"\
  1 "安卓手机"\
  2 "安卓手表"\
  3 "MacOS"\
  4 "企点"\
...
gcq_menu(){ [ -s config.yml ]||{ gcq_create||return;}
Choose="$(menubox "go-cqhttp $NAME ($VER)
账号：$GCQDir"\
  1 "打开 go-cqhttp"\
  2 "启动 go-cqhttp"\
  3 "停止 go-cqhttp"\
...
case "$Choose" in
  ...
  5)gcq_create;;
  6)gcq_device;;
  7)if [ -s qrcode.png ];then depend catimg&&catimg -t qrcode.png>&3&&back;else msgbox "未找到二维码文件";fi;;
  8)file_list;;
  9)gcq_download&&cd "$DIR/go-cqhttp/$GCQDir";;
  10)yesnobox "确认要清除缓存？"&&{ rm -vrf data logs&&msgbox "缓存清除完成"||abort "缓存清除失败";};;
  11)GCQDir="$(inputbox "请输入QQ号")"&&mkdir -vp "$DIR/go-cqhttp/$GCQDir"&&cd "$DIR/go-cqhttp/$GCQDir";;
  12)yesnobox "确认要删除账号？"&&{ rm -vrf "$DIR/go-cqhttp/$GCQDir"||abort "账号删除失败";return;};;
  13)fg_start "$GCQDir" go-cqhttp;back;;
  *)return
esac;gcq_menu;}

gcq(){ getver go-cqhttp||{ yesnobox "未安装 go-cqhttp，是否开始下载"&&gcq_download&&getver go-cqhttp||return;}
...
mcl_download(){ type java &>/dev/null||{
if [ -n "$MSYS" ];then
  depend_install java
else
...
case "$Choose" in
  ...
  6)file_list;;
  7)mcl_download;;
  8)yesnobox "确认要清除缓存？"&&{ rm -vrf data logs&&msgbox "缓存清除完成"||abort "缓存清除失败";};;
  9)yesnobox "确认要清除数据？"&&{ rm -vrf $(ls|rg -v '^(mcl\.jar|version)$')&&java -jar mcl.jar --update-package net.mamoe:mirai-api-http --channel stable-v2 --type plugin&&java -jar mcl.jar -uz&&msgbox "数据清除完成"||abort "数据清除失败";};;
  10)fg_start Mirai;back;;
  *)return
esac;mcl;}
...
Config_Input="$(inputbox "请输入 WebSocket 服务器地址" "ws://localhost:25365")"||return
Config_URL="$Config_Input"
Config_Server='{
      "Url": "'"$Config_Input"'",
      "AccessToken": ""
    }'
while yesnobox "是否继续添加 WebSocket 服务器"&&
  Config_Input="$(inputbox "请输入 WebSocket 服务器地址" "ws://localhost:")";do
  Config_URL="$Config_URL
$Config_Input"
  Config_Server="$Config_Server"',
    {
...
case "$Choose" in
  ...
  6)file_list;;
  7)zbp_download;;
  8)yesnobox "确认要清除缓存？"&&{ rm -vrf data&&msgbox "缓存清除完成"||abort "缓存清除失败";};;
  9)yesnobox "确认要清除数据？"&&{ rm -vrf $(ls|rg -v '^(zbp|version)$')&&msgbox "数据清除完成"||abort "数据清除失败";};;
  10)fg_start zbp ZeroBot;back;;
  *)return
esac;zbp;}
...
$Y- 正在下载 PaimengBot$O"
time_start
getver_github RicheyJang/PaimengBot pmb PaimengBot||return
case "$(uname -m)" in
  aarch64|arm64|armv8*|armv9*)ARCH=arm64;;
...
mktmp
if [ -n "$MSYS" ];then
  geturl "$URL/RicheyJang/PaimengBot/releases/download/$NEWNAME/PaimengBot-$NEWNAME-Windows_$ARCH.zip">"$TMP/pmb.zip"||abort "下载失败"
  unzip -o "$TMP/pmb.zip" -d "$TMP"||abort "解压失败"
else
  geturl "$URL/RicheyJang/PaimengBot/releases/download/$NEWNAME/PaimengBot-$NEWNAME-Linux_$ARCH.tar.gz">"$TMP/pmb.tgz"||abort "下载失败"
  tar -xvzf "$TMP/pmb.tgz" -C "$TMP"||abort "解压失败"
fi
[ -s pmb ]&&{ mv -vf pmb pmb.bak||abort "重命名原文件失败";}
cp -vrf "$TMP/paimengbot/"* .&&mv -vf paimeng pmb||abort "移动下载文件失败"
echo -n "name=$NEWNAME
version=$NEWVER
md5=$(md5 pmb)">version
time_stop
msgbox "PaimengBot 下载完成，用时：$TimeSpend";}

pmb_create(){ Config_SuperUser="$(inputbox "请输入主人QQ")"&&
Config_NickName="$(inputbox "请输入Bot昵称" 派蒙)"&&
Config_URL="$(inputbox "请输入 WebSocket 服务器地址" "ws://localhost:25365")"||return
echo "# 欢迎使用 TRSS OneBot ! 作者：时雨🌌星空
# 按 Ctrl+Q Y 保存退出
# 参考：https://richeyjang.github.io/PaimengBot/use/main
...
- $Config_SuperUser
tmp:
  maxcount: 1000">config-main.yaml||abort "配置文件写入失败"
msgbox "配置文件生成完成：
主人QQ：$Config_SuperUser
Bot昵称：$Config_NickName
连接地址：$Config_URL";}

pmb(){ getver pmb PaimengBot||{ yesnobox "未安装 PaimengBot，是否开始下载"&&pmb_download&&getver pmb PaimengBot||return;}
[ -s config-main.yaml ]||{ pmb_create||return;}
Choose="$(menubox "PaimengBot $NAME ($VER)"\
  1 "打开 PaimengBot"\
  2 "启动 PaimengBot"\
  3 "停止 PaimengBot"\
  4 "修改插件配置"\
  ...
  0 "返回")"
case "$Choose" in
  1)tmux_attach pmb PaimengBot;;
  2)tmux_start pmb PaimengBot;;
  3)tmux_stop pmb PaimengBot;;
  4)editor config-plugin.yaml;;
  5)editor config-main.yaml;;
  6)pmb_create;;
  7)file_list;;
  8)pmb_download;;
  9)yesnobox "确认要清除缓存？"&&{ rm -vrf data log&&msgbox "缓存清除完成"||abort "缓存清除失败";};;
  10)yesnobox "确认要清除数据？"&&{ rm -vrf $(ls|rg -v '^(pmb|version)$')&&msgbox "数据清除完成"||abort "数据清除失败";};;
  11)fg_start pmb PaimengBot;back;;
  *)return
esac;pmb;}
...
[ -d "$1" ]&&GitDir="$1"||return
if [ -d "$GitDir/.git" ];then Choose="$(menubox "- Git 插件：$GitDir ($(git_date "$GitDir"))"\
  1 "文件管理"\
  2 "删除插件"\
  3 "更新日志"\
  4 "更新插件"\
...
nb_git_plugin_choose(){ cd "$NBDir/$NBPluginDir"||return
...
rm -rf Liteyuki||process_stop "删除"
...
case "$Choose" in
  ...
  10)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  11)yesnobox "将会清除所有数据，确认要重新安装？"&&ly_download;;
  12)fg_start Liteyuki;back;;
  *)return
esac;ly;}
...
rm -rf LittlePaimon||process_stop "删除"
...
case "$Choose" in
  ...
  8)git_log;;
  9)git_update poetry install;back;;
  10)yesnobox "确认要重置项目？"&&{ git reset --hard&&rm -vrf .env.prod&&msgbox "项目重置完成"||abort "项目重置失败";};;
  11)yesnobox "将会清除所有数据，确认要重新安装？"&&lp_download;;
  12)fg_start LittlePaimon;back;;
  *)return
esac;lp;}
...
if [ -d "$GitDir/.git" ];then
  Choose="$(menubox "- Git 插件：$GitDir ($(git_date "$GitDir"))"\
  1 "文件管理"\
  2 "删除插件"\
  3 "软件包管理"\
  4 "更新日志"\
...
[ -d "$1" ]&&GitDir="$1"||return
if [ -d "$GitDir/.git" ];then Choose="$(menubox "- Py 插件 Git 插件：$GitDir ($(git_date "$GitDir"))"\
  1 "文件管理"\
  2 "删除插件"\
  3 "更新日志"\
  4 "更新插件"\
...
yz_py_plugin_git_choose(){ cd "$YzDir/plugins/py-plugin/plugins"||return
Choose="$(menubox "- 请选择插件"\
  1 "GenshinUID"\
  2 "LittlePaimon"\
  0 "自定义")"
...
poetry_install
rm -rf Real-ESRGAN||process_stop "删除"
git_clone "https://gitee.com/TimeRainStarSky/Real-ESRGAN"&&
cd Real-ESRGAN&&
...
poetry_install
rm -rf RemBG||process_stop "删除"
git_clone "https://gitee.com/TimeRainStarSky/RemBG"&&
cd RemBG&&
gitserver||return
mktmp
...
poetry_install
rm -rf ChatWaifu GenshinVoice||process_stop "删除"
git_clone "https://gitee.com/TimeRainStarSky/ChatWaifu"&&
git_clone "https://gitee.com/TimeRainStarSky/GenshinVoice"&&
...
yz_git_plugin_choose(){ cd "$YzDir/plugins"||return
Choose="$(menubox "- 请选择插件"\
  1 "Atlas                  图鉴插件"\
  2 "suiyue                 碎月插件"\
  3 "Icepray                冰祈插件"\
  ...
  9 "phi-plugin              phi插件"\
  10 "zhi-plugin             白纸插件"\
  11 "auto-plugin          自动化插件"\
  12 "k423-plugin            k423插件"\
  13 "miao-plugin            喵喵插件"\
  14 "mora-plugin            摩拉插件"\
  15 "WeLM-plugin            WeLM插件"\
  16 "armoe-plugin           贫困插件"\
  17 "ayaka-plugin           绫华插件"\
  18 "Guoba-Plugin           锅巴插件"\
  19 "sanyi-plugin           三一插件"\
  20 "voice-plugin           语音插件"\
  21 "yenai-plugin           椰奶插件"\
  22 "expand-plugin          拓展插件"\
  23 "flower-plugin          抽卡插件"\
  24 "paimon-plugin          派蒙插件"\
  25 "qianyu-plugin          千羽插件"\
  26 "XiaoXuePlugin          小雪插件"\
  27 "xiaoye-plugin          小叶插件"\
  28 "xitian-plugin        JS管理插件"\
  29 "chatgpt-plugin      ChatGPT插件"\
  30 "earth-k-plugin         土块插件"\
  31 "hs-qiqi-plugin         枫叶插件"\
  32 "liulian-plugin         榴莲插件"\
  33 "QQGuild-Plugin       QQ频道插件"\
  34 "windoge-plugin         风歌插件"\
  35 "xianxin-plugin         闲心插件"\
  36 "xiaofei-plugin         小飞插件"\
  37 "xiaoyue-plugin         小月插件"\
  38 "xiuxian-plugin         修仙插件"\
  39 "zhishui-plugin         止水插件"\
  40 "rconsole-plugin           R插件"\
  41 "Telegram-Plugin    Telegram插件"\
  42 "recreation-plugin      娱乐插件"\
  43 "yunzai-c-v-plugin    清凉图插件"\
  44 "xiaoyao-cvs-plugin     图鉴插件"\
  45 "achievements-plugin    成就插件"\
  46 "call_of_seven_saints   七圣召唤"\
  47 "ff14-composite-plugin  FF14插件"\
  48 "akasha-terminal-plugin 虚空插件"\
  49 "Jinmaocuicuisha-plugin 脆鲨插件"\
  0 "自定义")"
URL="https://gitee.com"
case "$Choose" in
  ...
  8)yz_git_plugin_install lin-plugin "$URL/go-farther-and-farther/lin-plugin";;
  9)gitserver&&yz_git_plugin_install phi-plugin "$URL/catrong/phi-plugin";;
  10)yz_git_plugin_install zhi-plugin "$URL/headmastertan/zhi-plugin";;
  11)yz_git_plugin_install auto-plugin "$URL/Nwflower/auto-plugin";;
  12)gitserver&&yz_git_plugin_install k423-plugin "$URL/K423-D/k423-plugin"&&pnpm_add axios btoa canvas;;
  13)yz_git_plugin_install miao-plugin "$URL/yoimiya-kokomi/miao-plugin";;
  14)yz_git_plugin_install mora-plugin "$URL/Rrrrrrray/mora-plugin";;
  15)yz_git_plugin_install WeLM-plugin "$URL/shuciqianye/yunzai-custom-dialogue-welm";;
  16)yz_git_plugin_install armoe-plugin "$URL/armoe-project/armoe-plugin";;
  17)gitserver&&yz_git_plugin_install ayaka-plugin "$URL/lumie-fx/ayaka-plugin";;
  18)yz_git_plugin_install Guoba-Plugin "$URL/guoba-yunzai/guoba-plugin";;
  19)yz_git_plugin_install sanyi-plugin "$URL/ThreeYi/sanyi-plugin";;
  20)gitserver&&yz_git_plugin_install voice-plugin "$URL/yuchiXiong/voice-plugin"&&pnpm_add cheerio;;
  21)yz_git_plugin_install yenai-plugin "$URL/yeyang52/yenai-plugin";;
  22)yz_git_plugin_install expand-plugin "$URL/SmallK111407/expand-plugin";;
  23)yz_git_plugin_install flower-plugin "$URL/Nwflower/flower-plugin";;
  24)gitserver&&yz_git_plugin_install paimon-plugin "$URL/zlh-debug/paimon-plugin";;
  25)yz_git_plugin_install qianyu-plugin "$URL/think-first-sxs/qianyu-plugin";;
  26)yz_git_plugin_install XiaoXuePlugin "$URL/XueWerY/XiaoXuePlugin";;
  27)yz_git_plugin_install xiaoye-plugin "$URL/xiaoye12123/xiaoye-plugin";;
  28)yz_git_plugin_install xitian-plugin "$URL/XiTianGame/xitian-plugin";;
  29)gitserver&&yz_git_plugin_install chatgpt-plugin "$URL/ikechan8370/chatgpt-plugin";;
  30)yz_git_plugin_install earth-k-plugin "$URL/SmallK111407/earth-k-plugin";;
  31)yz_git_plugin_install hs-qiqi-plugin "$URL/kesally/hs-qiqi-cv-plugin"&&pnpm_add axios;;
  32)yz_git_plugin_install liulian-plugin "$URL/huifeidemangguomao/liulian-plugin"&&pnpm_add axios;;
  33)gitserver&&yz_git_plugin_install QQGuild-Plugin "$URL/2y8e9h22/QQGuild-Plugin";;
  34)gitserver&&yz_git_plugin_install windoge-plugin "$URL/gxy12345/windoge-plugin";;
  35)yz_git_plugin_install xianxin-plugin "$URL/xianxincoder/xianxin-plugin"&&pnpm_add axios;;
  36)yz_git_plugin_install xiaofei-plugin "$URL/xfdown/xiaofei-plugin"&&pnpm_add axios;;
  37)yz_git_plugin_install xiaoyue-plugin "$URL/yunxiyuan/xiaoyue-plugin"&&pnpm_add axios;;
  38)yz_git_plugin_install xiuxian@2.0.0 "$URL/three-point-of-water/xiuxian-plugin";;
  39)yz_git_plugin_install zhishui-plugin "$URL/fjcq/zhishui-plugin"&&pnpm_add fluent-ffmpeg;;
  40)yz_git_plugin_install rconsole-plugin "$URL/kyrzy0416/rconsole-plugin"&&pnpm_add axios mongodb tunnel twitter-api-v2;;
  41)yz_git_plugin_install Telegram-Plugin "$URL/TimeRainStarSky/Yunzai-Telegram-Plugin";;
  42)gitserver&&yz_git_plugin_install recreation-plugin "$URL/QiuLing0/recreation-plugin";;
  43)yz_git_plugin_install yunzai-c-v-plugin "$URL/xwy231321/yunzai-c-v-plugin";;
  44)yz_git_plugin_install xiaoyao-cvs-plugin "$URL/Ctrlcvs/xiaoyao-cvs-plugin";;
  45)yz_git_plugin_install achievements-plugin "$URL/zolay-poi/achievements-plugin";;
  46)yz_git_plugin_install call_of_seven_saints "$URL/huangshx2001/call_of_seven_saints";;
  47)yz_git_plugin_install ff14-composite-plugin "$URL/jo30k/ff14-composite-plugin";;
  48)yz_git_plugin_install akasha-terminal-plugin "$URL/go-farther-and-farther/akasha-terminal-plugin";;
  49)yz_git_plugin_install Jinmaocuicuisha-plugin "$URL/JMCCS/jinmaocuicuisha";;
...
if [ -d "$GitDir/.git" ];then Choose="$(menubox "- Adachi 插件：$GitDir ($(git_date "$GitDir"))"\
  1 "文件管理"\
  2 "删除插件"\
  3 "软件包管理"\
  4 "更新日志"\
...
ac_plugin_choose(){ cd "$DIR/Adachi/src/plugins"||return
Choose="$(menubox "- 请选择插件"\
  1 "music                 点歌插件"\
  2 "hot-news              新闻插件"\
  3 "acg_search            以图识番"\
...
getver home/Dragonfly||{ yesnobox "未安装 Dragonfly，是否开始下载"&&dragonfly_download&&getver home/Dragonfly||plugin;}
...
case "$Choose" in
  ...
  3)yesnobox "确认要删除 Dragonfly？"&&
    rm -vrf "$HOME/Dragonfly";;
  *)return
esac;dragonfly;}
...
fix_version(){ Config_Device="$(menubox "- 请修改登录平台"\
  1 "安卓手机"\
  2 "安卓平板"\
  3 "安卓手表"\
  4 "MacOS"\
...
[ -s "$DeviceFile" ]||{ DeviceFile="$(ls data/icqq/*/device.json)"||{ msgbox "未找到设备文件";return;};}
if [ "$(wc -l<<<"$DeviceFile")" != 1 ];then
  Choose="$(eval menubox "'- 请选择设备文件' $(n=1;while read i;do echo -n "$n \"$i\" ";((n++));done<<<"$DeviceFile")")"||return
  DeviceFile="$(sed -n "${Choose}p"<<<"$DeviceFile")"
fi
gaugebox "- 正在生成随机设备文件"
sed_json(){ echo -n "s/\"$1\": *\".*\"/\"$1\": \"$2\"/;";}
sed -i "$(for i in product device board brand model wifi_ssid bootloader android_id;do
  sed_json "$i" "$(random_string 0-9a-zA-Z $[RANDOM%8+8])"
done
sed_json boot_id "$(uuidgen)"
sed_json proc_version "Linux version $(uname -r)"
sed_json mac_address "$(random_string 0-9A-F 12|sed "s/../&:/g"|head -c 17)"
sed_json ip_address "$[RANDOM%256].$[RANDOM%256].$[RANDOM%256].$[RANDOM%256]"
sed_json imei "$(random_string 0-9 15)")s/\"incremental\": .*,/\"incremental\": $(random_string 0-9 9),/" "$DeviceFile"
gaugebox_stop
rg -p "platform:" "$1"|bat --paging never - "$DeviceFile"
back;}
...
case "$Choose" in
  ...
  6)git remote set-url origin "https://gitee.com/TimeRainStarSky/Yunzai-Bot"&&git_update pnpm i -w icqq@latest&&fix_version config/config/qq.yaml data/device.json;;
  7)catimg_qrcode;;
  8)yz_plugin;;
  9)file_list;;
  10)git_log;;
  11)git_update pnpm i;back;;
  12)yesnobox "确认要清除数据？"&&{ rm -vrf config/config/* data logs&&msgbox "数据清除完成"||abort "数据清除失败";};;
  13)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  14)yesnobox "将会清除所有数据，确认要重新安装？"&&yz_download;;
  15)fg_start Yunzai;back;;
  *)return
esac;yz;}
...
rm -rf Miao-Yunzai||process_stop "删除"
...
case "$Choose" in
  ...
  4)file_list config/config;;
  5)rm -vrf config/config/*&&msgbox "配置文件已删除"||abort "配置文件删除失败";;
  6)git_update pnpm i -w icqq@latest&&fix_version config/config/qq.yaml data/icqq/device.json;;
  7)catimg_qrcode;;
  8)yz_plugin;;
  9)file_list;;
  10)git_log;;
  ...
  12)yesnobox "确认要清除数据？"&&{ rm -vrf config/config/* data logs&&msgbox "数据清除完成"||abort "数据清除失败";};;
  13)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  14)yesnobox "将会清除所有数据，确认要重新安装？"&&myz_download;;
  15)fg_start Miao-Yunzai;back;;
  *)return
esac;myz;}
...
rm -rf TRSS-Yunzai||process_stop "删除"
...
case "$Choose" in
  ...
  10)yesnobox "确认要清除数据？"&&{ rm -vrf config/config/* data logs&&msgbox "数据清除完成"||abort "数据清除失败";};;
  11)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  12)yesnobox "将会清除所有数据，确认要重新安装？"&&tyz_download;;
  13)fg_start TRSS-Yunzai;back;;
  *)return
esac;tyz;}
...
case "$Choose" in
  ...
  4)file_list config;;
  5)ac_create;;
  6)fix_version config/setting.yml;;
  7)catimg_qrcode;;
  8)ac_plugin;;
  9)file_list;;
  10)git_log;;
  11)git_update pnpm i -w puppeteer@19.2.2 @types/express-serve-static-core;back;;
  12)yesnobox "确认要清除数据？"&&{ rm -vrf data database logs&&msgbox "数据清除完成"||abort "数据清除失败";};;
  13)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  14)yesnobox "将会清除所有数据，确认要重新安装？"&&ac_download;;
  15)fg_start Adachi;back;;
  *)return
esac;ac;}
...
saya_plugin_choose(){ cd "$DIR/Sagiri/modules/third_party"||return
Choose="$(menubox "- 请选择插件"\
  1 "MockingBird"\
  0 "自定义")"
case "$Choose" in
...
case "$Choose" in
  ...
  9)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  10)yesnobox "将会清除所有数据，确认要重新安装？"&&si_download;;
  11)fg_start Sagiri;back;;
  *)return
esac;si;}
...
case "$Choose" in
  ...
  9)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  10)yesnobox "将会清除所有数据，确认要重新安装？"&&ai_download;;
  11)fg_start Amiya;back;;
  *)return
esac;ai;}
...
if [ -n "$MSYS" ];then
  ...
  rm -rf "$PGSQLDB"&&
  mkdir -vp "$PGSQLDB"&&
  initdb --locale zh_CN.UTF-8 -D "$PGSQLDB"||process_stop
  ...
  su - postgres -c "initdb --locale zh_CN.UTF-8 -D '$PGSQLDB'"&&
...
case "$Choose" in
  ...
  8)file_list;;
  9)git_log;;
  10)git_update "poetry install&&poetry run playwright install chromium";back;;
  11)yesnobox "确认要重置项目？"&&{ git reset --hard&&rm -vrf .env.dev&&msgbox "项目重置完成"||abort "项目重置失败";};;
  12)yesnobox "将会清除所有数据，确认要重新安装？"&&zx_download;;
  13)fg_start Zhenxun;back;;
  *)return
esac;zx;}
...
case "$Choose" in
  ...
  8)git_update;back;;
  9)yesnobox "确认要重置项目？"&&{ git reset --hard&&msgbox "项目重置完成"||abort "项目重置失败";};;
  10)yesnobox "将会清除所有数据，确认要重新安装？"&&zxwebui_download;;
  11)fg_start ZxWebUI;back;;
  *)return
esac;zxwebui;}
...
case "$Choose" in
  ...
  4)backup_choose PaimengBot PaimengBot-data PaimengBot/config* PaimengBot/data*;;
  5)backup_choose Liteyuki Liteyuki-data Liteyuki/.env* Liteyuki/data*;;
  6)backup_choose LittlePaimon LittlePaimon-data LittlePaimon/.env* LittlePaimon/config* LittlePaimon/data*;;
  7)backup_choose Yunzai Yunzai-data Yunzai/config/config* Yunzai/data*;;
  8)backup_choose Miao-Yunzai Miao-Yunzai-data Miao-Yunzai/config/config* Miao-Yunzai/data*;;
  9)backup_choose TRSS-Yunzai TRSS-Yunzai-data TRSS-Yunzai/config/config* TRSS-Yunzai/data*;;
  10)backup_choose Adachi Adachi-data Adachi/config* Adachi/data*;;
  11)backup_choose Sagiri Sagiri-data Sagiri/config* Sagiri/data*;;
  12)backup_choose Amiya Amiya-data Amiya/config* Amiya/data*;;
  13)backup_choose Zhenxun Zhenxun-data Zhenxun/.env* Zhenxun/config* Zhenxun/data*;;
  14)backup_zstd ZxWebUI;;
  15)backup_zstd home;;
  16)backup_zstd All home Adachi Amiya go-cqhttp Liteyuki LittlePaimon Miao-Yunzai Mirai PaimengBot Sagiri TRSS-Yunzai Yunzai ZeroBot Zhenxun ZxWebUI;;
...
[ "${Config[AutoStart_PaimengBot]}" = 1 ]&&echo "main PaimengBot start">>"$DIR/AutoStart.sh"
...
autostart(){ Choose="$(menubox "- 请选择自启动项目"\
  ...
  4 "PaimengBot   $([ "${Config[AutoStart_PaimengBot]}" = 1 ]&&echo "开启"||echo "关闭")"\
  5 "Liteyuki     $([ "${Config[AutoStart_Liteyuki]}" = 1 ]&&echo "开启"||echo "关闭")"\
  6 "LittlePaimon $([ "${Config[AutoStart_LittlePaimon]}" = 1 ]&&echo "开启"||echo "关闭")"\
  7 "Yunzai       $([ "${Config[AutoStart_Yunzai]}" = 1 ]&&echo "开启"||echo "关闭")"\
  8 "Miao-Yunzai  $([ "${Config[AutoStart_MiaoYunzai]}" = 1 ]&&echo "开启"||echo "关闭")"\
  9 "TRSS-Yunzai  $([ "${Config[AutoStart_TRSSYunzai]}" = 1 ]&&echo "开启"||echo "关闭")"\
  10 "Adachi       $([ "${Config[AutoStart_Adachi]}" = 1 ]&&echo "开启"||echo "关闭")"\
  11 "Sagiri       $([ "${Config[AutoStart_Sagiri]}" = 1 ]&&echo "开启"||echo "关闭")"\
  12 "Amiya        $([ "${Config[AutoStart_Amiya]}" = 1 ]&&echo "开启"||echo "关闭")"\
  13 "Zhenxun      $([ "${Config[AutoStart_Zhenxun]}" = 1 ]&&echo "开启"||echo "关闭")"\
  14 "ZxWebUI      $([ "${Config[AutoStart_ZxWebUI]}" = 1 ]&&echo "开启"||echo "关闭")")"
case "$Choose" in
  ...
  4)[ "${Config[AutoStart_PaimengBot]}" = 1 ]&&Config[AutoStart_PaimengBot]=||Config[AutoStart_PaimengBot]=1;;
  5)[ "${Config[AutoStart_Liteyuki]}" = 1 ]&&Config[AutoStart_Liteyuki]=||Config[AutoStart_Liteyuki]=1;;
  6)[ "${Config[AutoStart_LittlePaimon]}" = 1 ]&&Config[AutoStart_LittlePaimon]=||Config[AutoStart_LittlePaimon]=1;;
  7)[ "${Config[AutoStart_Yunzai]}" = 1 ]&&Config[AutoStart_Yunzai]=||Config[AutoStart_Yunzai]=1;;
  8)[ "${Config[AutoStart_MiaoYunzai]}" = 1 ]&&Config[AutoStart_MiaoYunzai]=||Config[AutoStart_MiaoYunzai]=1;;
  9)[ "${Config[AutoStart_TRSSYunzai]}" = 1 ]&&Config[AutoStart_TRSSYunzai]=||Config[AutoStart_TRSSYunzai]=1;;
  10)[ "${Config[AutoStart_Adachi]}" = 1 ]&&Config[AutoStart_Adachi]=||Config[AutoStart_Adachi]=1;;
  11)[ "${Config[AutoStart_Sagiri]}" = 1 ]&&Config[AutoStart_Sagiri]=||Config[AutoStart_Sagiri]=1;;
  12)[ "${Config[AutoStart_Amiya]}" = 1 ]&&Config[AutoStart_Amiya]=||Config[AutoStart_Amiya]=1;;
  13)[ "${Config[AutoStart_Zhenxun]}" = 1 ]&&Config[AutoStart_Zhenxun]=||Config[AutoStart_Zhenxun]=1;;
  14)[ "${Config[AutoStart_ZxWebUI]}" = 1 ]&&Config[AutoStart_ZxWebUI]=||Config[AutoStart_ZxWebUI]=1;;
  *)return
...
$C- 关于脚本：$O
  ...
  PaimengBot:$C https://richeyjang.github.io/PaimengBot$O
...
case "$1" in
  g|gcq|go-cqhttp)cd "$DIR/go-cqhttp/$2"&&case "$3" in
    a|attach)tmux_attach "$2" go-cqhttp;;
    s|start)tmux_start_quiet "$2" go-cqhttp;;
    f|fgstart)fg_start "$2" go-cqhttp;;
    st|stop)tmux_stop_quiet "$2" go-cqhttp;;
  ...
  p|pmb|PaimengBot)cd "$DIR/PaimengBot"&&case "$2" in
    a|attach)tmux_attach pmb PaimengBot;;
    s|start)tmux_start_quiet pmb PaimengBot;;
    f|fgstart)fg_start pmb PaimengBot;;
    st|stop)tmux_stop_quiet pmb PaimengBot;;
    cp|config-plugin)editor config-plugin.yaml;;
    c|config)editor config-main.yaml;;
    cr|create)pmb_create;;
    fi|file)file_list;;
    d|download|u|update)pmb_download;;
    *)pmb
  esac;;
  ...
  z|zx|Zhenxun)cd "$DIR/Zhenxun"&&case "$2" in
    ...
    u|update)git_update "poetry install&&poetry run playwright install chromium";;
    *)zx
  esac;;

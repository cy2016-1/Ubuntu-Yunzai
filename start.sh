...
pushd $HOME
clear
echo -e '\033[44;37m请允许存储与后台权\033[0m'
termux-setup-storage
sleep 3s
termux-wake-lock
sleep 2s
pkg install wget openssl-tool proot -y && hash -r
rm -rf ubuntu-binds ubuntu-fs ubuntu-rootfs.tar.xz start-ubuntu.sh
folder=ubuntu-fs
if [ -d "$folder" ]; then
	first=1
...
read -p "" num
        ...
        *)
		...
		wget "${ghproxy}https://raw.githubusercontent.com/EXALAB/AnLinux-Resources/master/Rootfs/Ubuntu/${archurl}/ubuntu-rootfs-${archurl}.tar.xz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"

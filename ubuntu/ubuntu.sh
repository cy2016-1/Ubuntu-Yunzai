...
pkg install -y openssl-tool pulseaudio proot aria2
echo 目前可用Ubuntu版本
echo ------------------
echo 1. ubuntu23
echo 2. ubuntu22
...
echo 4. ubuntu18
echo -------------------
echo -n 请输入您要安装的ubuntu版本的序号: ;read num
if [ ${num} = "1" ];then
ubuntu=lunar
ubuntu_version=23
elif [ ${num} = "2" ];then
...
echo -e ${red}您已安装ubuntu${ubuntu_version}${background}
exit
fi
...
grep -o 'title="[^"]*"' | \
awk -F'"' '{print $2}' )
if ! aria2c -o rootfs.tar.xz https://mirrors.bfsu.edu.cn/lxc-images/images/ubuntu/${ubuntu}/${structure}/default/${date}/rootfs.tar.xz
  then
    echo "下载失败 请检查网络!!"
    exit 1
fi
